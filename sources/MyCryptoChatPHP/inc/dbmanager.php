...
class DbManager {
    ...
    function CreateChatroom($chatRoom) {
        ...
        try {
			$query = 'INSERT INTO ChatRoom ( Id, DateCreation, DateLastNewMessage, ConnectedUsers, DateEnd, NoMoreThanOneVisitor, UserHash )';
			$query .= ' VALUES (?, ?, ?, ?, ?, ?, ?)';
			
			$req = $this->db->prepare($query);

			$req->execute(array($chatRoom->id, $chatRoom->dateCreation, $chatRoom->dateLastNewMessage, json_encode($chatRoom->users)
								, $chatRoom->dateEnd, $chatRoom->noMoreThanOneVisitor ? 1 : 0, $chatRoom->userId));
    ...
    function CleanChatrooms($time) {
        try {
            $query = 'DELETE FROM ChatMessage WHERE IdChatRoom IN (SELECT Id FROM ChatRoom WHERE DateEnd < ?)';
            
            $req = $this->db->prepare($query);
            
            $req->execute(array($time));
            
            $query = 'DELETE FROM ChatRoom WHERE DateEnd < ?';
    ...
    function GetChatroom($id) {
        ...
        try {            
            $query = 'SELECT Id, DateCreation, DateLastNewMessage, ConnectedUsers, DateEnd, NoMoreThanOneVisitor, UserHash FROM ChatRoom WHERE Id = ?';
